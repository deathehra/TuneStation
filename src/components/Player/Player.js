...
function Player(props) {
    ...
    useEffect(() => {
        ...
        document.title = `Playing ${props.details.name.replace(/&quot;/g, '"')} - DeathMusic`
    ...
    const handlePlayPause = () => {
        const audio = audioRef.current;
        if (isPlaying) {
            document.title = `Paused ${props.details.name.replace(/&quot;/g, '"')} - DeathMusic`
            audio.pause();
            setIsPlaying(false);
        } else {
            document.title = `Playing ${props.details.name.replace(/&quot;/g, '"')} - DeathMusic`
